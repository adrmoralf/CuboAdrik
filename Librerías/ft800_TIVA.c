...
void Inicia_pantalla(void)
{
    ...
    FT800_SPI_Write16(SWIZZLE);                     // Send the 16-bit value
...
void PantOnOff(char on)
{
    ...
    else
    {
        HAL_SPI_CSLow();                                // CS low                   Write REG_GPIO
        FT800_SPI_SendAddressWR(REG_GPIO);              // Send the address
        FT800_SPI_Write8(0x03);                         // Send the 8-bit value
        HAL_SPI_CSHigh();
        HAL_SPI_CSLow();
        FT800_SPI_SendAddressWR(REG_PWM_DUTY);
        FT800_SPI_Write8(0);
...
void Nueva_pantalla(int R, int G, int B)
{
    ...
    EscribeRam32(0x02000000+color);  // Clear Color RGB   00000010 BBBBBBBB GGGGGGGG RRRRRRRR  (B/G/R = Colour values)
    EscribeRam32(0x26000007);                            // Clear 00100110 -------- -------- -----CST  (C/S/T define which parameters to clear))
}
...
unsigned char Lee_Reg8(unsigned long dir)
{
    unsigned char Buff_temp;
    HAL_SPI_CSLow();                        //
    FT800_SPI_SendAddressRD(dir);   //
    Buff_temp = FT800_SPI_Read8();           // Lee valor
    HAL_SPI_CSHigh();
    return(Buff_temp);
}
...
void ComLine(int x1, int y1, int x2, int y2, int ancho)
{
    ...
    EscribeRam32(CMD_LINEWIDTH+16*ancho);
    EscribeRam32(CMD_BEGIN_LINES);
    ComVertex2ff(x1,y1);
    ComVertex2ff(x2,y2);
...
void ComRect(int x1, int y1, int x2, int y2, char relleno)
{
    ...
    EscribeRam32(CMD_SAVE_CONTEXT);

    if(relleno==1){
        EscribeRam32(CMD_LINEWIDTH+20);
        EscribeRam32(CMD_BEGIN_RECTS);
        ComVertex2ff(x1,y1);
        ComVertex2ff(x2,y2);
        EscribeRam32(CMD_END);
    }
    ...
    EscribeRam32(CMD_RESTORE_CONTEXT);
}
...
void ComCirculo(int x, int y, int r)
{
    ...
    EscribeRam32(CMD_SAVE_CONTEXT);
    EscribeRam32(CMD_POINTSIZE+r*16);// Point Size  00001101 00000000 SSSSSSSS SSSSSSSS  (S = Size value)
    EscribeRam32(CMD_BEGIN_POINTS);
    ComVertex2ff(x,y);
    EscribeRam32(CMD_END);
    EscribeRam32(CMD_RESTORE_CONTEXT);
}
...
void ComLineWidth(int width){
    EscribeRam32(CMD_LINEWIDTH+width*16);
}

void ComPointSize(int size){
    EscribeRam32(CMD_POINTSIZE+size*16);
}
...
void Calibra_touch(void)
{
    ComEsperaFin();
    EscribeRam32(CMD_DLSTART);
    EscribeRam32(0x02f7f7f7);  // Clear Color RGB   00000010 BBBBBBBB GGGGGGGG RRRRRRRR  (B/G/R = Colour values)
    EscribeRam32(0x26000007);                            // Clear 00100110 -------- -------- -----CST  (C/S/T define which parameters to clear))
    ...
    ComEsperaFin();
    EscribeRam32(CMD_DLSTART);
    EscribeRam32(0x02ffff00);  // Clear Color RGB   00000010 BBBBBBBB GGGGGGGG RRRRRRRR  (B/G/R = Colour values)
    EscribeRam32(0x26000007);                            // Clear 00100110 -------- -------- -----CST  (C/S/T define which parameters to clear))
    Dibuja();
}
